...
function createPopup(team) {
    ...
    const getStageName = (week) => {
        if (week >= 1 && week <= 8) return "Group Stage";
        if (week >= 9 && week <= 10) return "Play-Off";
        if (week >= 11 && week <= 12) return "Round Of 16";
        if (week >= 13 && week <= 14) return "Quarter Final";
        if (week >= 15 && week <= 16) return "Semi Final";

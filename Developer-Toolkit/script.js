...
function showMessage(message,type='error'){
    ...
    if(type==='error'){messageBox.classList.add('text-red-400');}
    else{messageBox.classList.add('text-green-400');}
...
function renderToolLayout(title,description,content){
    appContainer.innerHTML=`
        <div class="main-container">
            <header class="text-center">
                ...
                <button onclick="navigate(null)" class="text-[#00A389] hover:text-[#008775] mt-4 block text-sm mx-auto">← Tüm Araçlara Geri Dön</button>
            </header>
            <div class="card-area">
                ${content}
            </div>
    ...
    if(cardArea&&!cardArea.contains(messageBox)){cardArea.appendChild(messageBox);}
}
...
function updateUnitOptions(category,inputSelect,outputSelect){
    ...
    if(inputSelect.options.length>1){outputSelect.selectedIndex=1;}
}
function convertUnits(category,inputValue,inputUnitKey,outputUnitKey,outputValueElement){
    const value=parseFloat(inputValue);
    if(isNaN(value)){outputValueElement.value='';return showMessage('Lütfen geçerli bir sayı girin.', 'error');}
    let result;
    if(category==='temperature'){
        let baseValue;
        if(inputUnitKey==='F'){baseValue=(value-32)*(5/9);}
        else if(inputUnitKey==='K'){baseValue=value-273.15;}
        else{baseValue=value;}
        if(outputUnitKey==='F'){result=baseValue*(9/5)+32;}
        else if(outputUnitKey==='K'){result=baseValue+273.15;}
...
function renderUnitConverter(){
    ...
    renderToolLayout(tool.title,tool.description,`
        <div>
            <label for="category-select" class="label-text">Kategori Seçin:</label>
            <select id="category-select" class="select-style w-full p-3 rounded-lg text-base">
                <option value="length">Uzunluk</option>
                <option value="weight">Ağırlık / Kütle</option>
                <option value="temperature">Sıcaklık</option>
            </select>
        </div>
        <div id="conversion-fields" class="space-y-4">
            <div class="conversion-field">
                <input type="number" id="input-value" class="input-style p-3 rounded-lg" value="1" placeholder="Değer girin">
                <select id="input-unit" class="select-style p-3 rounded-lg"></select>
            </div>
            <div class="conversion-field">
                <input type="number" id="output-value" class="input-style p-3 rounded-lg bg-slate-700/50" readonly placeholder="Sonuç">
    ...
    const convert=()=>{convertUnits(categorySelect.value,inputValue.value,inputUnitSelect.value,outputUnitSelect.value,outputValue);};
    categorySelect.addEventListener('change',()=>{updateUnitOptions(categorySelect.value,inputUnitSelect,outputUnitSelect);convert();});
...
function renderTextCaseConverter(){
    ...
    renderToolLayout(tool.title,tool.description,`
        <div>
            <label for="input-text" class="label-text">Giriş Metni:</label>
            <textarea id="input-text" class="input-style w-full p-3 rounded-lg resize-y" rows="8" placeholder="Dönüştürmek istediğiniz metni buraya girin..."></textarea>
        </div>
        <div class="flex flex-col sm:flex-row gap-4 items-end">
            <div class="flex-1 w-full">
                <label for="action-select" class="label-text">Dönüşüm Tipi:</label>
                <select id="action-select" class="select-style w-full p-3 rounded-lg text-base">
                    ...
                    <option value="title">Başlık Durumu</option>
                    <option value="sentence">Cümle Durumu</option>
                    <option value="kebab">kebab-durumu</option>
                    <option value="snake">yılan_durumu</option>
                    <option value="camel">camelCase</option>
                </select>
            </div>
            <div class="w-full sm:w-auto">
                <button id="process-button" class="primary-button text-white font-bold py-3 px-6 rounded-lg w-full text-lg uppercase shadow-lg shadow-[#00A389]/30">DÖNÜŞTÜR</button>
            </div>
        </div>
        <div class="pt-4 border-t border-slate-700/50">
            <label for="output-text" class="label-text">Çıktı:</label>
            <textarea id="output-text" class="input-style w-full p-3 rounded-lg resize-y bg-slate-700/50" rows="8" readonly placeholder="Dönüştürülmüş metin buraya gelecektir."></textarea>
        </div>
    `);
    ...
    processButton.addEventListener('click',() => {
        ...
        if(text===''){outputTextarea.value='';return showMessage('Lütfen dönüştürmek için metin girin.', 'error');}
        try{
            const result=processTextConversion(text,action);
            outputTextarea.value=result;
            showMessage(`Metin ${action} durumuna başarıyla dönüştürüldü.`, 'success');
        }catch(e){
            outputTextarea.value=`Dönüşüm sırasında hata oluştu.`;
...
function renderBase64Converter(){
    ...
    renderToolLayout(tool.title,tool.description,`
        <div>
            <label for="input-text" class="label-text">Giriş Metni / Base64 Verisi:</label>
            <textarea id="input-text" class="input-style w-full p-3 rounded-lg resize-y" rows="8" placeholder="Metninizi veya Base64 dizenizi buraya yapıştırın..."></textarea>
        </div>
        <div class="flex flex-col sm:flex-row gap-4 items-end">
            <div class="flex-1 w-full">
                <label for="action-select" class="label-text">İşlem:</label>
                <select id="action-select" class="select-style w-full p-3 rounded-lg text-base">
                    <option value="encode">Kodla (Metin Base64'e)</option>
                    <option value="decode">Çöz (Base64 Metne)</option>
                </select>
            </div>
            <div class="w-full sm:w-auto">
                <button id="process-button" class="primary-button text-white font-bold py-3 px-6 rounded-lg w-full text-lg uppercase shadow-lg shadow-[#00A389]/30">İŞLEM YAP</button>
            </div>
        </div>
        <div class="pt-4 border-t border-slate-700/50">
            <label for="output-text" class="label-text">Çıktı:</label>
            <textarea id="output-text" class="input-style w-full p-3 rounded-lg resize-y bg-slate-700/50" rows="8" readonly placeholder="Sonuç buraya gelecektir."></textarea>
        </div>
    `);
    const inputTextarea=document.getElementById('input-text');
    const outputTextarea=document.getElementById('output-text');
    ...
    processButton.addEventListener('click',() => {
        const text=inputTextarea.value.trim();
        const action=actionSelect.value;
        if(text===''){outputTextarea.value='';return showMessage('Lütfen işlem yapmak için metin veya Base64 dizisi girin.', 'error');}
        try{
            let result='';
            if(action==='encode'){result=btoa(text);showMessage('Metin başarıyla kodlandı.', 'success');}
            else if(action==='decode'){result=atob(text);showMessage('Metin başarıyla çözüldü.', 'success');}
            outputTextarea.value=result;
        }catch(e){
            outputTextarea.value=`Veri işlenirken hata oluştu. Base64 dizisinin geçerli olup olmadığını veya ASCII olmayan karakter içerip içermediğini kontrol edin.`;
            showMessage('İşlem sırasında bir hata oluştu.', 'error');
        }
    });
...
function renderURLEncoder(){
    ...
    renderToolLayout(tool.title,tool.description,`
        <div>
            <label for="input-text" class="label-text">Giriş Metni / URL:</label>
            <textarea id="input-text" class="input-style w-full p-3 rounded-lg resize-y" rows="8" placeholder="Metninizi veya URL'nizi buraya yapıştırın..."></textarea>
        </div>
        <div class="flex flex-col sm:flex-row gap-4 items-end">
            <div class="flex-1 w-full">
                <label for="action-select" class="label-text">İşlem:</label>
                <select id="action-select" class="select-style w-full p-3 rounded-lg text-base">
                    <option value="encode">Kodla (Metin %-kaçışa)</option>
                    <option value="decode">Çöz (%-kaçış Metne)</option>
                </select>
            </div>
            <div class="w-full sm:w-auto">
                <button id="process-button" class="primary-button text-white font-bold py-3 px-6 rounded-lg w-full text-lg uppercase shadow-lg shadow-[#00A389]/30">İŞLEM YAP</button>
    ...
    processButton.addEventListener('click',() => {
        const text=inputTextarea.value.trim();
        const action=actionSelect.value;
        if(text===''){outputTextarea.value='';return showMessage('Lütfen işlem yapmak için metin veya URL girin.', 'error');}
        try{
            let result='';
            if(action==='encode'){result=encodeURIComponent(text);showMessage('Metin başarıyla kodlandı.', 'success');}
            else if(action==='decode'){result=decodeURIComponent(text);showMessage('Metin başarıyla çözüldü.', 'success');}
            outputTextarea.value=result;
        }catch(e){
            outputTextarea.value=`Veri işlenirken hata oluştu. Geçersiz bir URL dizesini çözmeye çalışıp çalışmadığınızı kontrol edin.`;
            showMessage('İşlem sırasında bir hata oluştu.', 'error');
        }
    });
...
function renderJsonXmlFormatter(){
    ...
    renderToolLayout(tool.title,tool.description,`
        <div>
            <label for="data-input" class="label-text">Giriş Verisi:</label>
            <textarea id="data-input" class="input-style w-full p-3 rounded-lg resize-y" rows="12" placeholder="JSON veya XML verinizi buraya yapıştırın..."></textarea>
        </div>
        <div class="flex flex-col sm:flex-row gap-4 items-end">
            <div class="flex-1 w-full">
                <label for="type-select" class="label-text">Veri Tipi Seçin:</label>
            ...
            <div class="w-full sm:w-auto">
                <button id="format-button" class="primary-button text-white font-bold py-3 px-6 rounded-lg w-full text-lg uppercase shadow-lg shadow-[#00A389]/30">FORMATLA & DOĞRULA</button>
            </div>
        </div>
        <div class="pt-4 border-t border-slate-700/50">
            <label for="data-output" class="label-text">Formatlanmış Çıktı:</label>
            <textarea id="data-output" class="input-style w-full p-3 rounded-lg resize-y bg-slate-700/50" rows="12" readonly placeholder="Formatlanmış ve doğrulanmış veri buraya gelecektir."></textarea>
        </div>
    `);
    ...
    formatButton.addEventListener('click',() => {
        ...
        let formattedData='';
        if(data===''){dataOutput.value='';return showMessage('Lütfen formatlamak için veri girin.', 'error');}
        try{
            if(dataType==='json'){
                const parsedJson=JSON.parse(data);
                formattedData=JSON.stringify(parsedJson,null,4);
                showMessage('JSON başarıyla doğrulandı ve formatlandı.', 'success');
            }else if(dataType==='xml'){
                const options={indent_size:4,space_in_empty_paren:true,};
                formattedData=window.html_beautify(data,options);
                showMessage('XML başarıyla formatlandı.', 'success');
            }
            dataOutput.value=formattedData;
        }catch(e){
            let errorMessage=`Doğrulama Hatası: Geçersiz ${dataType.toUpperCase()} sözdizimi.`;
            if(dataType==='json'){errorMessage+=` Virgül, küme parantezi veya geçersiz JSON yapılarını kontrol edin.`;}
            else if(dataType==='xml'){errorMessage+=` Etiket eşleşmesini veya iyi biçimlendirilmiş olmayan XML'i kontrol edin.`;}
...
function renderImageCompressor(){
    ...
    renderToolLayout(tool.title,tool.description,`
        <div>
            <label for="image-upload" class="label-text">Görüntü Yükle:</label>
            <div id="image-upload" class="file-upload-box flex justify-center items-center h-24 rounded-lg">
                <p id="file-status" class="text-slate-400">Dosyayı buraya sürükleyin veya tıklayın (JPG/PNG)</p>
            </div>
        ...
        <div id="image-controls" class="space-y-4 hidden">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="width-input" class="label-text">Yeni Genişlik (px):</label>
                    <input type="number" id="width-input" class="input-style w-full p-3 rounded-lg" value="800">
                </div>
                <div>
                    <label for="height-input" class="label-text">Yeni Yükseklik (px):</label>
                    <input type="number" id="height-input" class="input-style w-full p-3 rounded-lg" value="600">
                </div>
            </div>
            <div>
                <label for="quality-input" class="label-text">Kalite (Sıkıştırma, 0.1 - 1.0): <span id="quality-value">0.8</span></label>
                <input type="range" id="quality-input" min="0.1" max="1.0" step="0.05" value="0.8" class="slider-thumb w-full">
            </div>
            <button id="process-image-button" class="primary-button text-white font-bold py-3 px-6 rounded-lg w-full text-lg uppercase shadow-lg shadow-[#00A389]/30">YENİDEN BOYUTLANDIR & SIKIŞTIR</button>
        </div>
        <div id="image-output" class="pt-4 border-t border-slate-700/50 hidden">
            <p class="preview-title">Önizleme ve İndirme:</p>
            <img id="output-preview" class="image-preview" alt="İşlenmiş Görüntü">
            <p id="output-info" class="text-slate-400 mt-2 text-sm"></p>
            <a id="download-link" href="#" download="compressed_image.jpg" class="primary-button inline-block text-white font-bold py-2 px-4 rounded-lg mt-4 w-full text-center">İndir</a>
        </div>
    `);
    const fileInput=document.getElementById('file-input');
    ...
    imageUpload.addEventListener('dragover',(e) => {e.preventDefault();imageUpload.classList.add('border-[#00A389]');});
    imageUpload.addEventListener('dragleave',() => imageUpload.classList.remove('border-[#00A389]'));
    imageUpload.addEventListener('drop',(e) => {e.preventDefault();imageUpload.classList.remove('border-[#00A389]');if(e.dataTransfer.files.length)handleFileSelect({target:{files:e.dataTransfer.files}});});
    ...
    function handleFileSelect(event){
        ...
        if(!originalFile||!originalFile.type.match('image.*')){showMessage('Lütfen geçerli bir resim dosyası (JPG/PNG) seçin.', 'error');return;}
        fileStatus.textContent=`Seçilen dosya: ${originalFile.name}`;
    ...
    function processImage(){
        if(!originalFile){showMessage('Lütfen önce bir görüntü dosyası yükleyin.', 'error');return;}
        const newWidth=parseInt(widthInput.value);
        const newHeight=parseInt(heightInput.value);
        ...
        reader.onload=(e) => {
            ...
            img.onload=() => {
                ...
                canvas.toBlob((blob) => {
                    ...
                    outputInfo.textContent=`Yeni boyut: ${newWidth}x${newHeight} px. Yeni dosya boyutu: ${(blob.size/1024).toFixed(2)} KB.`;
                    imageOutput.classList.remove('hidden');
                    showMessage('Görüntü başarıyla işlendi.', 'success');
                },outputType,quality);
            };
            img.src=e.target.result;
        };
...
function renderColorPalette(){
    ...
    renderToolLayout(tool.title,tool.description,`
        <div>
            <label for="base-color-input" class="label-text">Ana Renk (HEX):</label>
        ...
        <div id="palette-output" class="pt-4 border-t border-slate-700/50">
            <p class="preview-title">Oluşturulan Palet:</p>
    ...
    const updateUI=(hex) => {
        ...
        if(!palette){showMessage('Lütfen geçerli bir HEX kodu girin.', 'error');return;}
        ...
        document.getElementById('base-color-area').innerHTML=`
            <h4 class="text-sm font-medium text-slate-300 mb-2">Ana Renk:</h4>
        ...
        `;
        renderSwatch('Tamamlayıcı (Complementary)',palette.complementary,'complementary-area');
        renderSwatch('Benzer (Analogous)',palette.analogous,'analogous-area');
        renderSwatch('Üçlü (Triadic)',palette.triadic,'triadic-area');
    };
    ...
    baseColorInput.addEventListener('input',(e) => {
        ...
        if(hex.length===7&&hex.startsWith('#')){updateUI(hex);}

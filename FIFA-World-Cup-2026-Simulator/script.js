...
const FLAG_URL = (code) => `https://api.fifa.com/api/v3/picture/flags-sq-2/${code}`;
const TEAM_CODES = {
    'Mexico': 'MEX',
    'S. Korea': 'KOR',
    'S. Africa': 'RSA',
...
//     "Germany": 8, "Belgium": 9, "Italy": 10, "Croatia": 11, "Uruguay": 12, "USA": 13, "Mexico": 14,
//     "Colombia": 15, "Morocco": 16, "Switzerland": 17, "Denmark": 18, "Japan": 19, "Senegal": 20,
//     "Iran": 21, "S. Korea": 22, "Australia": 23, "Ecuador": 24, "Saudi Arabia": 25, "Ghana": 26,
//     "Turkey": 28, "Poland": 29, "Ukraine": 30, "Sweden": 31, "Romania": 32, "Paraguay": 33, 
//     "Austria": 34, "Norway": 35, "Egypt": 36, "Algeria": 37, "Qatar": 38, "Jamaica": 39, "Uzbekistan": 40, 
//     "DR Congo": 41, "Haiti": 42, "Bolivia": 43, "Suriname": 44, "Iraq": 45, "Curacao": 46, "N. Zealand": 47, 
//     "S. Africa": 48, "Wales": 49, "Bosnia & H.": 50, "N. Ireland": 51, "Albania": 52, "Slovakia": 53, 
//     "Kosovo": 54, "Rep. of Ireland": 55, "N. Macedonia": 56, "Cape Verde": 57, "Jordan": 58, "Panama": 59,
//     "N. Caledonia": 60, "TBD": 100 
// };
const initialGroups = {
    ...
    if (bestThirdPlace.length === 8) {
        ...
        if (matchups) {
            ...
            for (const [winnerSlot, thirdSlot] of Object.entries(matchups)) {
                const group = thirdSlot.charAt(1); // "E"
                ...
                if (team && tSlot) {
                    ...
                    console.warn(`Eşleşme hatası: ${winnerSlot} -> ${thirdSlot} (Takım: ${team?.Name}, Slot: ${tSlot})`);
                }
            }
            ...
            allTSlots.forEach(tSlot => {
                if (!qualifiers[tSlot]) {
                    console.error(`Eksik T slot'u: ${tSlot}`);
            ...
            console.warn(`Kombinasyon bulunamadı: ${key}, eski yöntem kullanılıyor.`);
...
const resolveTeam = (slotId) => {
    const qualifiers = currentQualifiers;
    if (slotId.length === 2) {
        return qualifiers[slotId] || 'TBD';
    }
    if (slotId.endsWith('W') && slotId.length > 2) {
        const baseMatchId = slotId.substring(0, slotId.length - 1);
        return KnockoutResults[baseMatchId] || 'TBD';
    }
    return KnockoutResults[slotId] || 'TBD';
};
...
function renderMatchNode(slot) {
    ...
    return `
                <div class="match-node" id="match-node-${slot.id}">
                    <div class="team-option ${currentWinner === team1Name ? 'winner' : (currentWinner && currentWinner !== '') ? 'loser' : ''} ${team1Name === 'TBD' ? 'tbd' : ''}" 
                         onclick="${isTBD ? '' : team1Click}" data-teamname="${team1Name}">
                        ${getTeamFlag(team1Name)} <span>${team1Name}</span>
                    </div>
                    <div class="team-option ${currentWinner === team2Name ? 'winner' : (currentWinner && currentWinner !== '') ? 'loser' : ''} ${team2Name === 'TBD' ? 'tbd' : ''}" 
                         onclick="${isTBD ? '' : team2Click}" data-teamname="${team2Name}">
                ...
                <div class="third-place-section" id="round-third">
                    ...
                    <div class="round-matches">
                        <div class="third-place-node match-node" id="match-node-T32">
                            <div class="team-option ${winner3rd === S29L ? 'winner' : (winner3rd && winner3rd !== '') ? 'loser' : ''} ${S29L === 'TBD' ? 'tbd' : ''}" 
                                onclick="${isTBD ? '' : team3rd1Click}" data-teamname="${S29L}">
                                ${getTeamFlag(S29L)} <span>${S29L}</span>
                            </div>
                            <div class="team-option ${winner3rd === S30L ? 'winner' : (winner3rd && winner3rd !== '') ? 'loser' : ''} ${S30L === 'TBD' ? 'tbd' : ''}" 
                                onclick="${isTBD ? '' : team3rd2Click}" data-teamname="${S30L}">
                ...
                <div class="final-section" id="round-final">
                    ...
                    <div class="round-matches">
                        <div class="final-match-node match-node" id="match-node-F31">
                            <div class="team-option ${winnerFinal === SFWinners[0] ? 'winner' : (winnerFinal && winnerFinal !== '') ? 'loser' : ''} ${SFWinners[0] === 'TBD' ? 'tbd' : ''}" 
                                onclick="${isTBD ? '' : teamFinal1Click}" data-teamname="${SFWinners[0]}">
                                ${getTeamFlag(SFWinners[0])} <span>${SFWinners[0]}</span>
                            </div>
                            <div class="team-option ${winnerFinal === SFWinners[1] ? 'winner' : (winnerFinal && winnerFinal !== '') ? 'loser' : ''} ${SFWinners[1] === 'TBD' ? 'tbd' : ''}" 
                                onclick="${isTBD ? '' : teamFinal2Click}" data-teamname="${SFWinners[1]}">
...
function renderKnockout() {
    ...
    const rounds = [{
            id: 'r32',
            title: 'Round of 32',
            slots: KNOCKOUT_SLOTS.slice(0, 16)
        },
        {
            id: 'r16',
            title: 'Round of 16',
            slots: KNOCKOUT_SLOTS.slice(16, 24)
        },
        {
            id: 'qf',
            title: 'Quarter-Finals',
            slots: KNOCKOUT_SLOTS.slice(24, 28)
        },
        {
            id: 'sf',
            title: 'Semi-Finals',
            slots: KNOCKOUT_SLOTS.slice(28, 30)
        }
    ];
    ...
    rounds.forEach(round => {
        let roundHTML = `
                    <div class="round-section" id="round-${round.id}">
                        <h3>${round.title}</h3>
                        <div class="round-matches">
                `;
        ...
        if (previousRoundCompleted) {
            ...
            container.innerHTML += `
                        <div class="round-section" id="round-${round.id}">
                            <h3>${round.title}</h3>
                            <p style="color: red; font-weight: bold;">The previous round was not completed.</p>
                        </div>
                    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', function(e) {
        if (e.target && e.target.id === 'fillRandomBtn') {
            fillRandomScores();
        }
    });
});
